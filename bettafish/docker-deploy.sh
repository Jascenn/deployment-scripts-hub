        ...
        if command_exists brew; then
            ...
            if brew install --cask docker; then
                ...
                if [ "$DOCKER_READY" = true ]; then
                    ...
                    echo ""
                    echo -e "  ${CYAN}解决方案 1 - 手动启动 Docker Desktop（推荐用于本地开发）:${NC}"
                    echo "    1. 在 Applications 中找到 Docker 并打开"
                    echo "    2. 接受服务协议 (Service Agreement)"
                    echo "    3. 授权 Docker 访问系统（输入密码）"
                    echo "    4. 等待状态栏显示 🐋 图标"
                    echo -e "    5. 重新运行: ${CYAN}bash docker-deploy.sh${NC}"
                    echo ""
                    echo -e "  ${CYAN}解决方案 2 - 使用 Colima（推荐用于 CI/测试环境）:${NC}"
...
if prepare_docker_image "python:3.11-slim" "python-3.11-slim.tar"; then
        ...
        if [[ "$DEPLOY_CHOICE" == "1" ]]; then
            ...
            if [ ! -d "$OFFLINE_DIR" ] || [ -z "$(ls -A $OFFLINE_DIR/*.tar 2>/dev/null)" ]; then
                ...
                echo ""
                echo -e "  ${CYAN}./download-offline-packages.sh${NC}"
                echo ""
                log_info "或手动准备离线包到目录："
                echo -e "  ${CYAN}$OFFLINE_DIR${NC}"
                echo ""
                exit 1
            fi
            ...
            if [ -f "$SCRIPT_DIR/download-offline-packages.sh" ]; then
                ...
                echo ""
                echo -e "  ${CYAN}./docker-deploy.sh${NC}"
                echo ""
                exit 0
            else
                ...
                log_info "请手动下载镜像："
                echo ""
                echo -e "  ${CYAN}docker pull python:3.11-slim${NC}"
                echo -e "  ${CYAN}docker pull postgres:15-alpine${NC}"
                echo ""
                echo -e "  ${CYAN}mkdir -p $OFFLINE_DIR${NC}"
                echo -e "  ${CYAN}docker save python:3.11-slim -o $OFFLINE_DIR/python-3.11-slim.tar${NC}"
                echo -e "  ${CYAN}docker save postgres:15-alpine -o $OFFLINE_DIR/postgres-15-alpine.tar${NC}"
                echo ""
                exit 1
            fi
...
if [ -z "$PROJECT_DIR" ]; then
    ...
    if [ "$DOWNLOAD_SUCCESS" = false ]; then
        ...
        log_info "请手动下载项目:"
        echo ""
        echo -e "  ${CYAN}# 方式 1: wget (推荐)${NC}"
        echo -e "  ${CYAN}wget https://github.com/666ghj/BettaFish/archive/refs/heads/main.zip${NC}"
        echo -e "  ${CYAN}unzip main.zip${NC}"
        echo ""
        echo -e "  ${CYAN}# 方式 2: curl${NC}"
        echo -e "  ${CYAN}curl -L https://github.com/666ghj/BettaFish/archive/refs/heads/main.tar.gz | tar xz${NC}"
        echo ""
        echo -e "  ${CYAN}# 方式 3: git clone${NC}"
        echo -e "  ${CYAN}git clone https://github.com/666ghj/BettaFish.git BettaFish-main${NC}"
        echo ""
        exit 1
    fi
fi
...
if [ ${#UNUSED_IMAGES[@]} -gt 0 ]; then
    ...
    if [[ "$CLEAN_IMAGES" =~ ^[Yy]$ ]]; then
        ...
        echo ""
        for image in "${UNUSED_IMAGES[@]}"; do
            echo -e "  ${CYAN}docker rmi $image${NC}"
        done
        echo ""
    fi
else
