# BettaFish 快速参考卡片

> 打印出来贴在电脑旁，随时查阅！

---

## 🚀 一键部署（首次使用）

```bash
cd ~/Downloads/BettaFish-Deployment-Kit
chmod +x docker-deploy.sh
./docker-deploy.sh
```

**按提示输入：**
1. 主 API 密钥（必填）
2. Tavily 密钥（推荐）
3. 选择镜像源（直接回车用推荐）

---

## 🌐 访问地址

| 服务 | 地址 | 用途 |
|------|------|------|
| **主服务** | http://localhost:5001 | 综合功能（推荐） |
| Insight Engine | http://localhost:8501 | 深度分析 |
| Media Engine | http://localhost:8502 | 媒体处理 |
| Query Engine | http://localhost:8503 | 快速问答 |

---

## ⚙️ 日常操作命令

### 启动服务
```bash
cd ~/Downloads/BettaFish-Deployment-Kit/BettaFish-main
docker-compose up -d
```
⏱️ 等待 30 秒后访问

### 停止服务
```bash
cd ~/Downloads/BettaFish-Deployment-Kit/BettaFish-main
docker-compose stop
```

### 查看状态
```bash
cd ~/Downloads/BettaFish-Deployment-Kit/BettaFish-main
docker-compose ps
```
✅ 正常显示：`Up X minutes`

### 查看日志
```bash
cd ~/Downloads/BettaFish-Deployment-Kit/BettaFish-main
docker-compose logs -f bettafish
```
按 `Ctrl + C` 退出

### 重启服务
```bash
cd ~/Downloads/BettaFish-Deployment-Kit/BettaFish-main
docker-compose restart
```

---

## 🔧 常见问题快速解决

| 问题 | 解决方法 |
|------|----------|
| 🔴 无法访问 | 1. 检查 Docker 是否运行<br>2. 等待 30 秒<br>3. 运行 `docker-compose ps` |
| 🔴 端口被占用 | 重新运行 `./docker-deploy.sh` |
| 🔴 下载失败 | 换镜像源或检查网络 |
| 🔴 服务异常 | 运行 `docker-compose restart` |
| 🔴 API 错误 | 检查密钥是否正确 |

---

## 📊 系统检查命令

### 检查 Docker 状态
```bash
docker --version
docker ps
```

### 检查端口占用
```bash
# macOS/Linux
lsof -i :5001

# 查看所有相关端口
lsof -i :5001 -i :8501 -i :8502 -i :8503
```

### 检查磁盘空间
```bash
df -h
```

### 检查内存使用
```bash
# macOS
top

# Linux
free -h
```

---

## 🗑️ 清理和维护

### 清理 Docker 缓存
```bash
# 清理未使用的镜像
docker image prune -a

# 清理未使用的容器
docker container prune

# 清理所有未使用资源
docker system prune -a
```

### 完全卸载
```bash
cd ~/Downloads/BettaFish-Deployment-Kit/BettaFish-main
docker-compose down -v
docker rmi ghcr.io/666ghj/bettafish:latest
docker rmi registry.cn-hangzhou.aliyuncs.com/library/postgres:15-alpine
cd ~
rm -rf ~/Downloads/BettaFish-Deployment-Kit
```

---

## 🔄 更新到最新版

```bash
cd ~/Downloads/BettaFish-Deployment-Kit
./docker-deploy.sh
# 选择 "Y" 使用现有配置
```

---

## 📁 重要文件位置

| 文件/目录 | 位置 | 说明 |
|-----------|------|------|
| 配置文件 | `BettaFish-main/.env` | API 密钥等配置 |
| 数据库 | `BettaFish-main/db_data/` | 对话历史 |
| 日志 | `logs/` | 部署日志 |
| 备份 | `backups/` | 配置备份 |
| 报告 | `final_reports/` | 生成的报告 |

---

## 🆘 紧急问题处理

### 服务无响应
```bash
# 强制重启
cd ~/Downloads/BettaFish-Deployment-Kit/BettaFish-main
docker-compose down
docker-compose up -d
```

### Docker 无法启动
```bash
# macOS: 重启 Docker Desktop
# 或者重启电脑
```

### 配置损坏
```bash
# 恢复备份
cd ~/Downloads/BettaFish-Deployment-Kit
cp backups/docker-compose_backup_最新时间.yml BettaFish-main/docker-compose.yml
```

### 数据库问题
```bash
# 重置数据库（会清空数据！）
cd ~/Downloads/BettaFish-Deployment-Kit/BettaFish-main
docker-compose down -v
rm -rf db_data/
docker-compose up -d
```

---

## 💡 使用技巧

### 1. 后台运行
```bash
# 启动后关闭终端也能运行
docker-compose up -d
```

### 2. 实时监控
```bash
# 实时查看日志
docker-compose logs -f
```

### 3. 性能优化
```bash
# 限制内存使用（在 docker-compose.yml 中添加）
mem_limit: 4g
```

### 4. 定时备份数据库
```bash
# 创建备份脚本
cat > backup.sh << 'EOF'
#!/bin/bash
DATE=$(date +%Y%m%d_%H%M%S)
docker exec bettafish-db pg_dump -U bettafish bettafish > backup_$DATE.sql
EOF
chmod +x backup.sh
```

---

## 📞 获取帮助

### 查看详细文档
```bash
cd ~/Downloads/BettaFish-Deployment-Kit
open 用户使用手册.md
open docs/guides/快速开始指南.md
```

### 在线支持
- GitHub Issues: 报告问题
- GitHub Discussions: 社区交流

---

## 🎯 记住这些！

✅ **启动前检查**：Docker 已启动
✅ **首次访问**：等待 30-60 秒
✅ **定期维护**：清理 Docker 缓存
✅ **备份重要**：定期备份数据库
✅ **更新及时**：关注新版本发布

---

**最重要的命令（记住这个就够了）：**

```bash
cd ~/Downloads/BettaFish-Deployment-Kit/BettaFish-main && docker-compose up -d
```

然后访问：http://localhost:5001

**就这么简单！** 🎉
